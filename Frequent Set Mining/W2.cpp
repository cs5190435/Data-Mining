...
int main(int argc, char *argv[]){
    ...
    if(!basket.is_open()){
        ...
        for(int cut= 4; cut>= 0; cut--){
            ...
            vector<freq> fset;
            sup[cut] = sup[cut]*buckets;          //Chnaging the given %sup to freq.
            sup[cut] = sup[cut]/100;
            //cout << sup[cut] << "\n";
            for(int i= 1; i< itemset.size(); i++){
                if(itemset[i] >= sup[cut]){
                    struct freq temp;
                    vector<int> it(1, i);
                    temp.iset = it;
                    temp.freq = itemset[i];
            ...
            for(int j= counter; j>= 0; j--){
                ...
                ftree(root,fset, sup[cut], outp, j, header);
                helper.pop_back();
            }
            delete(root);

            auto end = high_resolution_clock::now();
            fptime.push_back(duration_cast<microseconds>(end-start).count()*0.001);
            
            //cout << "comp-fp" << "\n";

            start = high_resolution_clock::now();
            apriori(fset, sup[cut] , file ,  outp);
            end = high_resolution_clock::now();
            aptime.push_back(duration_cast<microseconds>(end-start).count()*0.001);
            ofstream wf;
            wf.open("times.txt");
            for(int ct = 0; ct < aptime.size(); ct++){
                wf << aptime[ct] << " ";
            }
            wf << "\n";
            for(int ct = 0; ct < fptime.size(); ct++){
                wf << fptime[ct] << " ";
            }
            wf << "\n";
